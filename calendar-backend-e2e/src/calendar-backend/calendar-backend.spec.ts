...
describe('GET /api/calendar', () => {
  ...
  beforeEach(async () => {
    const {data: { id: id1 }} = await axios.post(`${baseUrl}/create-event`, {name: 'Event 1', start: '2024-10-07T13:00:00', end: '2024-10-07T14:00:00'});
    eventIds.push(id1);

    const {data: { id: id2 }} = await axios.post(`${baseUrl}/create-event`, {name: 'Event 2', start: '2024-10-08T15:00:00', end: '2024-10-08T16:00:00'});
    eventIds.push(id2);
  });
  ...
  describe('date-range', () => {
    it('should return bookings within range', async () => {
      ...
      const { status, data } = await axios.get(`${baseUrl}/date-range`, {params: {start, end}});
    ...
    it('should fail if start not specified', async () => {
      const end = '2025-10-08T00:00:00';
      const { status, data } = await axios.get(`${baseUrl}/date-range`, {params: {end}, validateStatus: () => true});
    ...
    it('should fail if end not specified', async () => {
      ...
      const { status, data } = await axios.get(`${baseUrl}/date-range`, {params: {start}, validateStatus: () => true});
